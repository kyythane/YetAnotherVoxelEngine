...
using Assets.SunsetIsland.Initialization;
...
namespace Assets.SunsetIsland.UI
{
    public class MainMenu : MonoBehaviour
    {
        ...
        private MD5 _hash;
        public GameObject IntroScene, NewGameOptions;
        ...
        [UsedImplicitly]
        public void DisplayNewGameOptions()
        {
        ...
        {
            SeedField.text = GetRandomSeed();
        }

        [NotNull]
        private static string GetRandomSeed()
        {
            var adjective1 = Adjectives[Random.Range(0, Adjectives.Count)];
            var adjective2 = Adjectives[Random.Range(0, Adjectives.Count)];
            var noun = Nouns[Random.Range(0, Nouns.Count)];
            return $"{adjective1} {adjective2} {noun}";
        }

        private uint GetHash([NotNull] string str)
        {
            var bytes = _hash.ComputeHash(Encoding.UTF8.GetBytes(str));
            var upper = ((uint) bytes[7] << 24) | ((uint) bytes[6] << 16) | ((uint) bytes[5] << 8) | bytes[4];
            var lower = ((uint) bytes[3] << 24) | ((uint) bytes[2] << 16) | ((uint) bytes[1] << 8) | bytes[0];
            return upper ^ lower;
        }
        ...
        public void StartGame()
        {
            ...
            {
                SeedName = str,
                SeedValue = GetHash(str)
            };
            var loader = new SimulationLoader(settings);
            loader.Run();
        }
    }
}