    ...
    public class CellNoise3D
    {
        ...
        public float VoroniFbm(float x, float y, float z, float scale, float offset, float octaves)
        {
            ...
            for (var i = 0; i < octaves; ++i)
            {
                ...
                rValue += 1.0f /
                          octaveScale *
                          (VoroniBlend(px * octaveScale, py * octaveScale, pz * octaveScale) + offset);
            }
        ...
        private void AddSamples(int x, int y, int z, int maxOrder, ref Vector3 at, Vector3[] deltas,
                                float[] basisFunctions, uint[] id)
        {
