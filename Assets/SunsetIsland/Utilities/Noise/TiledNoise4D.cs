...
{
    public class TiledNoise4D : TiledPseudoNoise
    {
        public TiledNoise4D(ulong seed)
            : base(seed, 4)
        {
        }

        public float RidgedMultiFractal(float x, float y, float z, float w, float scale, float offset, float gain,
                                        int octaves)
        {
            float rValue = 0;
        ...
        public float Perlin(Vector4 pos)
        {
            return Perlin(pos.x, pos.y, pos.z, pos.w);
        }

        public float Perlin(double pX, double pY, double pZ, double pW)
        {
            ...
            return (sampleX + sampleY + sampleZ) / 4;
        }
    }
}